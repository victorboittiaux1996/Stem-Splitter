...
import {
  ...
  AudioLines, BarChart3, Gamepad2, SkipBack, RotateCcw,
} from "lucide-react";
...
const NAV_COLORS: Record<string, string> = {
  split: "#FF2D55", files: "#FF9500", stats: "#34C759", games: "#AF52DE", settings: "#007AFF",
};
...
const HISTORY = [
  { id: "1", name: "Baime - Human Needs [BULKMIXMSTRV2]", date: "7h ago", stems: 4, duration: "5m 41s", format: "wav", bpm: 128, key: "Am",
...
const BpmTap = dynamic(() => import("./games/bpm-tap").then(m => ({ default: m.BpmTap })), { ssr: false });
const MpcPad = dynamic(() => import("./games/mpc-pad").then(m => ({ default: m.MpcPad })), { ssr: false });
const MelodyMemory = dynamic(() => import("./games/melody-memory"), { ssr: false });
const FrequencyQuiz = dynamic(() => import("./games/frequency-quiz"), { ssr: false });
const GuessTheStem = dynamic(() => import("./games/guess-the-stem").then(m => ({ default: m.GuessTheStem })), { ssr: false });
const TomatoToss = dynamic(() => import("./games/tomato-toss").then(m => ({ default: m.TomatoToss })), { ssr: false });
...
export default function AbletonDashboard() {
  ...
  const handleSplit = useCallback(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (p >= 100) { p = 100; setProgress(100); setStage("Complete!"); if (intervalRef.current) clearInterval(intervalRef.current); setTimeout(() => setAppState("complete"), 500); return; }
      setProgress(p);
      const newIdx = Math.min(STAGES.length - 1, Math.floor(p / (100 / STAGES.length)));
      if (newIdx !== stageIdx) { stageIdx = newIdx; setStage(STAGES[stageIdx]); }
    }, 180);
  }, [file]);
  const handleNewSplit = useCallback(() => { setFile(null); setAppState("idle"); setProgress(0); setStage(""); setIsPlaying(false); setCurrentTime(0); setSoloTrack(null); setMutedTracks(new Set()); }, []);
  ...
  const durToSec = (d: string) => { const m = d.match(/(\d+)m\s*(\d+)s/); return m ? parseInt(m[1]) * 60 + parseInt(m[2]) : 0; };
  ...
  // ─── Stem detail modal ──────────────────────────────────────
  function StemModal({ items, onClose }: { items: typeof HISTORY; onClose: () => void }) {
      ...
      <motion.div key="stem-modal" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }}
          ...
          <div className="flex items-center gap-[10px] px-[24px] py-[8px]" style={{ backgroundColor: C.bgSubtle }}>
            <span style={{ fontSize: 10, color: C.textMuted, letterSpacing: "0.05em" }}>SETTINGS:</span>
            <span style={{ fontSize: 11, fontWeight: 600, color: C.accent }}>AI</span>
            <span style={{ fontSize: 11, color: C.textSec }}>{currentItem.model}</span>
            <span style={{ fontSize: 11, color: C.textMuted }}>·</span>
            <span style={{ fontSize: 11, color: C.textSec }}>Quality {currentItem.quality}%</span>
            <span style={{ fontSize: 11, color: C.textMuted }}>·</span>
            <span style={{ fontSize: 11, color: C.textSec }}>Stability {currentItem.stability}%</span>
            <div className="flex-1" />
            <button style={{ fontSize: 11, color: C.textMuted, textDecoration: "underline", letterSpacing: "0.03em" }}>USE AS DEFAULT</button>
          </div>
          {/* Stems */}
            ...
            {currentItem.stemList.map((stem, si) => {
              ...
              return (
                <motion.div key={stem} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.04 * si }}
                  ...
                  <span className="w-[90px] shrink-0" style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.04em", color: C.text }}>{LABELS[stem]}</span>
                  ...
                  <span style={{ fontSize: 11, color: C.textMuted }}>{currentItem.format.toUpperCase()}</span>
          ...
          <div className="flex items-center justify-between px-[24px] py-[12px]" style={{ backgroundColor: C.bgSubtle }}>
            <span style={{ fontSize: 12, color: C.textMuted }}>{currentIdx + 1} / {items.length}</span>
            <button className="flex items-center gap-[6px] px-[16px] py-[7px] transition-colors"
              style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.04em", color: C.accentText, backgroundColor: C.accent }}>
      ...
      <aside className="flex h-full w-[220px] shrink-0 flex-col" style={{ backgroundColor: C.sidebarBg }}>
        ...
        <div className="relative">
          ...
          {versionOpen && (
            <div className="absolute left-[8px] right-[8px] top-[52px] z-50 py-[4px]" style={{ backgroundColor: C.bgCard }}>
              ...
              <Link href="/" onClick={() => setVersionOpen(false)}
              ...
              <div className="flex items-center gap-[8px] px-[12px] py-[8px]"
                style={{ color: C.text, fontSize: 13, fontWeight: 500, backgroundColor: C.navActive }}>
                <Layers className="h-[14px] w-[14px]" strokeWidth={1.6} />
                ABLETON
              </div>
            </div>
          ...
          {([
            { id: "split" as View, label: "SPLIT AUDIO", svg: (c: string) => (
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <path d="M2 8h12M5 4v8M8 2v12M11 5v6" stroke={c} strokeWidth="1.2" />
              </svg>
            )},
            { id: "files" as View, label: "MY FILES", svg: (c: string) => (
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <rect x="2" y="6" width="12" height="8" stroke={c} strokeWidth="1.2" />
                <path d="M4 6V4h8v2M6 4V2h4v2" stroke={c} strokeWidth="1.2" />
              </svg>
            )},
            { id: "stats" as View, label: "STATISTICS", svg: (c: string) => (
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <rect x="1" y="8" width="3" height="6" fill={c} />
                <rect x="5" y="4" width="3" height="10" fill={c} />
                <rect x="9" y="6" width="3" height="8" fill={c} />
                <rect x="13" y="2" width="2" height="12" fill={c} />
              </svg>
            )},
            { id: "games" as View, label: "GAMES", svg: (c: string) => (
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <rect x="2" y="2" width="12" height="12" stroke={c} strokeWidth="1.2" />
            ...
            { id: "settings" as View, label: "SETTINGS", svg: (c: string) => (
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <path d="M2 4h12M2 8h12M2 12h12" stroke={c} strokeWidth="1.2" />
                <rect x="4" y="2.5" width="3" height="3" fill={c} />
                <rect x="9" y="6.5" width="3" height="3" fill={c} />
                <rect x="5" y="10.5" width="3" height="3" fill={c} />
              </svg>
            )},
          ]).map(item => {
        ...
        <div style={{ backgroundColor: C.bgSubtle }} className="px-[8px] py-[6px] space-y-[1px]">
          <button onClick={() => switchTheme(() => setStyle(style === "classic" ? "colorful" : "classic"))}
            className="flex w-full items-center gap-[10px] px-[10px] py-[9px] transition-colors"
            style={{ fontSize: 13, fontWeight: 500, letterSpacing: "0.04em", color: isColorful ? "#FF9500" : C.textSec }}>
          ...
          <button className="flex w-full items-center gap-[10px] px-[10px] py-[9px] transition-colors"
            style={{ fontSize: 13, fontWeight: 500, letterSpacing: "0.04em", color: C.textSec }}>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex h-[52px] shrink-0 items-center justify-between px-[24px]" style={{ backgroundColor: C.bgSubtle }}>
          ...
          <div className="flex items-center gap-[4px]">
            {/* Pills */}
            <button className="px-[12px] py-[6px] transition-colors" style={{ fontSize: 12, fontWeight: 500, letterSpacing: "0.04em", color: C.textSec, backgroundColor: C.bgHover }}>FEEDBACK</button>
            <button className="px-[12px] py-[6px] transition-colors" style={{ fontSize: 12, fontWeight: 500, letterSpacing: "0.04em", color: C.textSec, backgroundColor: C.bgHover }}>DOCS</button>
            <button className="flex items-center gap-[4px] px-[12px] py-[6px] transition-colors" style={{ fontSize: 12, fontWeight: 500, letterSpacing: "0.04em", color: C.textSec, backgroundColor: C.bgHover }}>
        ...
        <div className="flex-1 overflow-y-auto">
              ...
              <div style={{ maxWidth: 900, margin: "0 auto" }}>
                ...
                {(appState === "idle" || appState === "file-selected") && (
                  <>
                    ...
                    <div
                      ...
                      style={{ minHeight: 160, backgroundColor: C.dropZoneBg, cursor: file ? "default" : "pointer" }}>
                      ...
                      {file ? (
                        <div className="flex flex-col items-center gap-[8px]">
                          ...
                          <p style={{ fontSize: 12, color: C.textMuted }}>{(file.size / 1048576).toFixed(1)} MB</p>
                          <button onClick={(e) => { e.stopPropagation(); setFile(null); setAppState("idle"); }}
                            style={{ fontSize: 12, color: C.textMuted, textDecoration: "underline", letterSpacing: "0.03em" }}>REMOVE</button>
                        </div>
                      ) : (
                        <span style={{ fontSize: 14, color: C.textMuted, letterSpacing: "0.02em" }}>DROP FILES HERE</span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between" style={{ marginTop: 12 }}>
                      <div className="flex items-center gap-[4px]">
                        <button onClick={() => inputRef.current?.click()} className="p-[8px] transition-colors" style={{ color: C.textSec }}>
                        ...
                        <div className="relative" ref={stemsRef}>
                          <button onClick={() => { setStemsOpen(!stemsOpen); setFormatOpen(false); setExtraOpen(false); }}
                            ...
                            style={{ fontSize: 13, fontWeight: 500, color: C.textSec, letterSpacing: "0.03em", backgroundColor: stemsOpen ? C.bgHover : undefined }}>
                            {stemLabel}
                              ...
                              <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }}
                                ...
                                {STEM_OPTIONS.map(opt => (
                                  <button key={opt.value} onClick={() => { setStemCount(opt.value); setStemsOpen(false); }}
                                    ...
                                    <div>
                                      <p style={{ fontSize: 13, fontWeight: 600, letterSpacing: "0.02em", color: C.text }}>{opt.label.toUpperCase()}</p>
                        ...
                        <div className="relative" ref={formatRef}>
                          <button onClick={() => { setFormatOpen(!formatOpen); setStemsOpen(false); setExtraOpen(false); }}
                            className="flex items-center gap-[4px] px-[8px] py-[6px] transition-colors"
                            style={{ fontSize: 13, fontWeight: 500, color: C.textSec, letterSpacing: "0.03em", backgroundColor: formatOpen ? C.bgHover : undefined }}>
                              ...
                              <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }}
                                ...
                                {([["wav", "WAV (LOSSLESS)"], ["mp3", "MP3 (128KBPS)"]] as const).map(([val, label]) => (
                                  <button key={val} onClick={() => { setOutputFormat(val as OutputFormat); setFormatOpen(false); }}
                                    ...
                                    style={{ fontSize: 13, fontWeight: outputFormat === val ? 600 : 400, backgroundColor: outputFormat === val ? C.bgHover : undefined, color: C.text, letterSpacing: "0.02em" }}>
                                    {label}
                                  </button>
                                ))}
                              </motion.div>
                            ...
                            {extraOpen && (
                              <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }}
                                ...
                                <div className="px-[16px] py-[12px]" style={{ backgroundColor: C.bgHover }}>
                                  <span style={{ fontSize: 13, fontWeight: 600, letterSpacing: "0.04em" }}>ADVANCED SETTINGS</span>
                                </div>
                                <div className="px-[16px] py-[14px] space-y-[16px]">
                                  <div className="space-y-[6px]">
                                    <label style={{ fontSize: 11, fontWeight: 600, color: C.textMuted, letterSpacing: "0.04em" }}>MODEL</label>
                                  ...
                                  <div className="space-y-[6px]">
                                    <label style={{ fontSize: 11, fontWeight: 600, color: C.textMuted, letterSpacing: "0.04em" }}>STABILITY</label>
                                  ...
                                  <div className="space-y-[6px]">
                                    <label style={{ fontSize: 11, fontWeight: 600, color: C.textMuted, letterSpacing: "0.04em" }}>QUALITY</label>
                                  ...
                                  <div className="flex items-center justify-between">
                                    <span style={{ fontSize: 12, letterSpacing: "0.03em" }}>CLEAN VOCALS</span>
                      ...
                      <div className="flex items-center gap-[10px]">
                        <span style={{ fontSize: 12, color: C.textMuted }}>0 / 10,000 credits</span>
                        <button onClick={handleSplit} disabled={!file}
                          ...
                          style={{ backgroundColor: file ? C.accent : C.textMuted, color: C.accentText, fontSize: 13, fontWeight: 600, letterSpacing: "0.03em" }}>
                          SPLIT
                        </button>
                      </div>
                    </div>
                    ...
                    <div style={{ marginTop: 40 }}>
                      ...
                      <div style={{ backgroundColor: C.bgCard }}>
                        ...
                        <div className="flex items-center px-[16px] py-[10px]" style={{ color: C.textMuted, fontSize: 12, fontWeight: 500, letterSpacing: "0.04em", borderBottom: `1px solid ${C.bgHover}` }}>
                        ...
                        {HISTORY.map((item, i) => (
                          <motion.div key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.03 * i }}
                            ...
                            <span className="w-[80px] text-right" style={{ fontSize: 14, color: C.textMuted }}>{item.format.toUpperCase()}</span>
                ...
                {appState === "complete" && (
                  <div className="space-y-[12px]">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <h2 style={{ fontSize: 20, fontWeight: 700, letterSpacing: "0.02em", color: C.text }}>SEPARATION COMPLETE</h2>
                        <p style={{ fontSize: 13, color: C.textMuted, marginTop: 2 }}>{file?.name} — {stemCount} stems</p>
                      </div>
                      <button onClick={handleNewSplit} className="flex items-center gap-[6px] px-[14px] py-[8px] transition-colors"
                        style={{ fontSize: 13, fontWeight: 500, color: C.textMuted, backgroundColor: C.bgHover, letterSpacing: "0.03em" }}>
                    ...
                    {/* Transport bar */}
                    <div className="flex items-center gap-[12px] px-[16px] py-[12px]" style={{ backgroundColor: C.bgCard }}>
                      <button onClick={() => { setCurrentTime(0); }} className="flex h-[32px] w-[32px] items-center justify-center" style={{ color: C.textMuted }}>
                        <SkipBack className="h-[14px] w-[14px]" />
                      </button>
                      <button onClick={() => setIsPlaying(!isPlaying)}
                        className="flex h-[36px] w-[36px] items-center justify-center transition-transform active:scale-95"
                        style={{ backgroundColor: C.text, color: C.bg }}>
                        {isPlaying ? <Pause className="h-[14px] w-[14px]" /> : <Play className="h-[14px] w-[14px] ml-[1px]" />}
                      </button>
                      <span className="w-[100px] text-center tabular-nums" style={{ fontSize: 13, color: C.textMuted }}>
                        {formatTime(currentTime)} / {formatTime(duration)}
                      </span>
                      <div className="relative flex-1 h-[4px] cursor-pointer" style={{ backgroundColor: C.bgHover }}
                        onClick={(e) => { const r = e.currentTarget.getBoundingClientRect(); setCurrentTime(Math.max(0, Math.min(1, (e.clientX - r.left) / r.width)) * duration); }}>
                        <div className="absolute inset-y-0 left-0" style={{ width: `${playProgress * 100}%`, backgroundColor: C.text }} />
                      </div>
                      <button className="flex items-center gap-[6px] px-[14px] py-[7px]"
                        style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.04em", color: C.accentText, backgroundColor: C.text }}>
                        <Download className="h-[12px] w-[12px]" strokeWidth={1.8} />
                        DOWNLOAD ALL
                      </button>
                    </div>
                    ...
                    <div className="space-y-[2px]">
                      {STEM_MAP[stemCount].map((name, i) => {
                        const config = STEM_CONFIGS[name] || { label: name, color: "#6B7280", playedColor: "#4B5563" };
                        const isMuted = mutedTracks.has(name);
                        const isSolo = soloTrack === name;
                        const isActive = soloTrack ? isSolo : !isMuted;
                        return (
                          <motion.div key={name} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 * i }}
                            className="flex items-center gap-[12px] px-[16px] py-[10px] transition-all"
                            style={{ backgroundColor: C.bgCard, opacity: isActive ? 1 : 0.4 }}>
                            <div className="flex w-[80px] shrink-0 items-center gap-[8px]">
                              <div className="h-[8px] w-[8px] shrink-0" style={{ backgroundColor: config.color }} />
                              <span style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.04em", color: C.text }}>{LABELS[name]}</span>
                            </div>
                            <div className="flex gap-[2px] shrink-0">
                              <button onClick={() => setSoloTrack(prev => prev === name ? null : name)}
                                className="px-[6px] py-[2px] text-[10px] font-bold tracking-wide transition-colors"
                                style={{ backgroundColor: isSolo ? "#F59E0B" : C.bgHover, color: isSolo ? "#451A03" : C.textMuted }}>S</button>
                              <button onClick={() => { if (soloTrack === name) { setSoloTrack(null); return; } setMutedTracks(prev => { const n = new Set(prev); n.has(name) ? n.delete(name) : n.add(name); return n; }); }}
                                className="px-[6px] py-[2px] text-[10px] font-bold tracking-wide transition-colors"
                                style={{ backgroundColor: isMuted ? "#F43F5E" : C.bgHover, color: isMuted ? "#fff" : C.textMuted }}>M</button>
                            </div>
                            <div className="flex-1 min-w-0">
                              <Waveform seed={(i + 1) * 7919 + 42} color={config.color} playedColor={config.playedColor} progress={playProgress} height={40}
                                onSeek={(p) => setCurrentTime(p * duration)} barCount={160} />
                            </div>
                            <button className="shrink-0 p-[6px]" style={{ color: C.textMuted }}>
                              <Download className="h-[13px] w-[13px]" strokeWidth={1.5} />
                            </button>
                          </motion.div>
                        );
              ...
              <div className="px-[24px] pt-[24px] pb-[40px]">
                <div style={{ maxWidth: 900, margin: "0 auto" }}>
                    ...
                    <div className="flex items-center gap-[8px]">
                      {exportMode && (
                        <button onClick={exitExport} className="px-[14px] py-[8px] transition-colors"
                          style={{ fontSize: 13, fontWeight: 500, color: C.textMuted, letterSpacing: "0.03em" }}>CANCEL</button>
                      )}
                  ...
                  <div style={{ backgroundColor: C.bgCard, overflow: "hidden" }}>
                    ...
                    {/* Column headers */}
                    <div className="flex items-center px-[16px] py-[10px] select-none" style={{ color: C.textMuted, fontSize: 12, fontWeight: 500, letterSpacing: "0.04em", borderBottom: `1px solid ${C.bgHover}` }}>
                      {exportMode && (
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                      ...
                      <div className="flex items-center justify-between px-[20px] py-[16px]" style={{ backgroundColor: C.bgHover }}>
                        <div>
                          <p style={{ fontSize: 14, fontWeight: 600, color: C.text, letterSpacing: "0.03em" }}>EXPORT STEMS</p>
                          <p style={{ fontSize: 12, color: C.textMuted, marginTop: 2 }}>{selectedTracks.size} track{selectedTracks.size > 1 ? "s" : ""} selected</p>
                        </div>
                      ...
                      <div className="px-[20px] py-[16px] space-y-[2px]">
                        <button onClick={toggleAllStems}
                          ...
                          <span style={{ fontSize: 13, fontWeight: 600, color: C.text, letterSpacing: "0.03em" }}>ALL STEMS</span>
                        </button>
                        {allStemTypes.map(stem => {
                          ...
                          return (
                            <button key={stem} onClick={() => toggleExportStem(stem)}
                              ...
                              <span style={{ fontSize: 13, fontWeight: 500, color: C.text, letterSpacing: "0.03em" }}>{LABELS[stem]}</span>
                            </button>
                          );
                        })}
                      </div>
                      <div className="flex items-center justify-end gap-[8px] px-[20px] py-[14px]" style={{ backgroundColor: C.bgSubtle }}>
                        <button onClick={() => setExportStemPicker(false)}
                          ...
                          style={{ fontSize: 13, fontWeight: 500, color: C.textSec, letterSpacing: "0.03em" }}>CANCEL</button>
                        <button onClick={() => { setExportStemPicker(false); exitExport(); }}
                          ...
                          style={{ fontSize: 13, fontWeight: 600, color: C.accentText, backgroundColor: C.text, letterSpacing: "0.03em" }}>
          ...
          {view === "stats" && (() => {
            ...
            const keyCounts: Record<string, number> = {}; HISTORY.forEach(h => { keyCounts[h.key] = (keyCounts[h.key] || 0) + 1; });
              ...
              <div className="px-[24px] pt-[24px] pb-[40px]">
                <div style={{ maxWidth: 900, margin: "0 auto" }}>
                  ...
                  <div className="grid grid-cols-4 gap-[8px] mb-[24px]">
                    {stats.map((stat, i) => (
                      <motion.div key={stat.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.04 * i }}
                        ...
                        <p style={{ fontSize: 10, fontWeight: 600, color: C.textMuted, letterSpacing: "0.05em" }}>{stat.label}</p>
                        <p style={{ fontSize: 28, fontWeight: 700, color: C.text, marginTop: 6, letterSpacing: "-0.02em" }}>{stat.value}</p>
                        <p style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>{stat.sub}</p>
                      </motion.div>
                    ))}
                  </div>
                  {/* Stem distribution */}
                  <div className="mb-[16px]" style={{ backgroundColor: C.bgCard }}>
                    <div className="px-[16px] py-[12px]" style={{ backgroundColor: C.bgHover }}>
                      <p style={{ fontSize: 13, fontWeight: 600, color: C.text, letterSpacing: "0.03em" }}>STEM DISTRIBUTION</p>
                    </div>
                      ...
                      {stemDistribution.map(([stem, count]) => {
                        ...
                        return (
                          <div key={stem} className="flex items-center gap-[12px]">
                            <span className="w-[80px] shrink-0" style={{ fontSize: 12, fontWeight: 600, color: C.text, letterSpacing: "0.04em" }}>{LABELS[stem]}</span>
                            <div className="flex-1 h-[6px] overflow-hidden" style={{ backgroundColor: C.bgHover }}>
                              <motion.div initial={{ width: 0 }} animate={{ width: `${(count / maxStemCount) * 100}%` }}
                                transition={{ duration: 0.6, delay: 0.1 }} className="h-full" style={{ backgroundColor: color }} />
                            </div>
                            <span className="w-[24px] text-right" style={{ fontSize: 12, color: C.textMuted }}>{count}</span>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <div style={{ backgroundColor: C.bgCard }}>
                    <div className="px-[16px] py-[12px]" style={{ backgroundColor: C.bgHover }}>
                      <p style={{ fontSize: 13, fontWeight: 600, color: C.text, letterSpacing: "0.03em" }}>FORMAT BREAKDOWN</p>
                    </div>
                      ...
                      {Object.entries(formatCounts).sort((a, b) => b[1] - a[1]).map(([fmt, count]) => {
                        ...
                        return (
                          <div key={fmt} className="flex flex-col items-center gap-[6px]">
                            ...
                            <span style={{ fontSize: 12, fontWeight: 600, color: C.textSec, letterSpacing: "0.04em" }}>{fmt.toUpperCase()}</span>
                          </div>
                        );
                      })}
                    </div>
              ...
              <div style={{ maxWidth: 900, margin: "0 auto", width: "100%" }}>
                {activeGame === "" ? (
                  <>
                    ...
                    <p style={{ fontSize: 13, color: C.textMuted, marginBottom: 24, letterSpacing: "0.02em" }}>TAKE A BREAK BETWEEN SPLITS</p>
                    <div className="grid grid-cols-3 gap-[8px]">
                      {[
                        { id: "bpm", name: "BPM TAP", desc: "Tap the tempo of famous tracks", letter: "B" },
                        { id: "tomato", name: "TOMATO TOSS", desc: "Throw tomatoes at a bad DJ", letter: "T" },
                        { id: "mpc", name: "MPC PAD", desc: "Play beats on a drum machine", letter: "M" },
                        { id: "melody", name: "MELODY MEMORY", desc: "Replay melodies from memory", letter: "♪" },
                        { id: "freq", name: "FREQUENCY QUIZ", desc: "Guess the frequency of a tone", letter: "Hz" },
                        { id: "stem", name: "GUESS THE STEM", desc: "Which stem is playing?", letter: "S" },
                      ].map(g => (
                        <button key={g.id} onClick={() => setActiveGame(g.id)}
                          ...
                          <div style={{
                            ...
                            backgroundColor: C.bgElevated, color: C.accent, fontSize: 16, fontWeight: 700,
                            fontFamily: F, letterSpacing: "0.02em",
                          }}>{g.letter}</div>
                          <p style={{ fontSize: 14, fontWeight: 600, color: C.text, marginTop: 10, letterSpacing: "0.03em" }}>{g.name}</p>
                          <p style={{ fontSize: 11, color: C.textMuted, marginTop: 4, lineHeight: 1.4 }}>{g.desc}</p>
                        </button>
                      ))}
                    </div>
                  </>
                  ...
                  <div>
                    <button onClick={() => setActiveGame("")}
                      ...
                      style={{ fontSize: 12, fontWeight: 500, color: C.textMuted, backgroundColor: C.bgHover, letterSpacing: "0.04em" }}>
            ...
            <div className="px-[24px] pt-[24px] pb-[40px]">
              <div style={{ maxWidth: 900, margin: "0 auto" }}>
                ...
                <div className="flex flex-col items-center justify-center py-[80px]">
                  ...
                  <p style={{ fontSize: 13, color: C.textMuted, marginTop: 12, letterSpacing: "0.03em" }}>ACCOUNT SETTINGS COMING SOON</p>
                </div>
              </div>
            </div>
          )}
    ...
    <div className="flex flex-col items-center justify-center px-8 py-[60px]">
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="text-center space-y-[8px]">
          ...
          <p style={{ fontSize: 13, fontWeight: 600, letterSpacing: "0.12em", ...shimmerStyle }}>
            {shimmerMsg}
          </p>
        </div>
            ...
            {PROCESSING_AGENTS.map((agent, i) => {
              ...
              return (
                <motion.div key={agent.name} layout
                  ...
                  <span style={{ fontSize: 13, fontWeight: isActive ? 500 : 400, color: isPrev ? C.textMuted : C.textSec, letterSpacing: "0.03em" }}>
        ...
        <div className="pt-[8px] flex justify-center">
          {!gameActive ? (
            <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2 }}
              ...
              className="px-[16px] py-[8px] transition-colors"
              style={{ fontSize: 12, color: C.textMuted, backgroundColor: C.bgHover, letterSpacing: "0.04em" }}>
              BORED? TAP THE BPM
            </motion.button>
          ) : (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="flex items-center justify-between mb-[16px]">
                <span style={{ fontSize: 11, color: C.textMuted, letterSpacing: "0.04em" }}>SCORE: {score}</span>
                {streak >= 2 && <span style={{ fontSize: 11, color: C.accent, letterSpacing: "0.04em" }}>STREAK {streak}</span>}
                <span style={{ fontSize: 11, color: C.textMuted, letterSpacing: "0.04em" }}>ROUND {totalRounds + 1}</span>
              </div>
              <p style={{ fontSize: 18, fontWeight: 700, color: C.text }}>{song.title}</p>
              <p style={{ fontSize: 13, color: C.textMuted, marginTop: 4 }}>{song.artist}</p>
                ...
                {!result ? (
                  <motion.div key="tap" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <button onClick={handleTap}
                      ...
                      <p style={{ fontSize: 13, fontWeight: 600, color: C.textSec, letterSpacing: "0.04em" }}>
                      ...
                      {taps.length > 0 && taps.length < 4 && (
                        <p style={{ fontSize: 11, color: C.textMuted, marginTop: 6 }}>{4 - taps.length} MORE TAPS NEEDED</p>
                      )}
                    </button>
                    <p style={{ fontSize: 11, color: C.textMuted, marginTop: 8, letterSpacing: "0.03em" }}>SPACE TO TAP · ENTER TO SUBMIT</p>
                    {userBpm !== null && (
                      <button onClick={submitGuess}
                        className="mt-[12px] px-[20px] py-[8px] transition-all"
                        style={{ fontSize: 13, fontWeight: 600, color: C.accentText, backgroundColor: C.accent, letterSpacing: "0.03em" }}>
                        LOCK IN {userBpm} BPM
                      </button>
                    )}
                  </motion.div>
