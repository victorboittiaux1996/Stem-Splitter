...
const PROJECTILES = [
  { id: "tomato", emoji: "\u{1F345}", label: "Tomato", splatColor: "rgba(229,62,62,0.5)" },
  { id: "egg", emoji: "\u{1F95A}", label: "Egg", splatColor: "rgba(250,230,100,0.5)" },
  { id: "shoe", emoji: "\u{1F45F}", label: "Shoe", splatColor: "rgba(139,92,246,0.4)" },
  { id: "pie", emoji: "\u{1F967}", label: "Pie", splatColor: "rgba(245,158,11,0.5)" },
  { id: "poop", emoji: "\u{1F4A9}", label: "Poop", splatColor: "rgba(120,80,40,0.5)" },
];

const VENUES: { id: Venue; label: string; emoji: string }[] = [
  { id: "clean", label: "Clean", emoji: "\u{1F3B5}" },
  { id: "neon", label: "Neon", emoji: "\u{1F49C}" },
  { id: "festival", label: "Festival", emoji: "\u{1F3AA}" },
  { id: "basement", label: "Cave", emoji: "\u{1F56F}\uFE0F" },
];
...
// ── Component ──
export function TomatoToss({ isDark, isColorful, theme }: GameProps) {
  ...
  return (
    <div style={{ width: "100%", maxWidth: 500, margin: "0 auto" }}>
      <div style={{ padding: 24, backgroundColor: theme.bgCard }}>
        {/* Header */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: theme.text }}>{score}</span>
            {combo >= 2 && (
              <span style={{ fontSize: 11, color: theme.accent, fontWeight: 700, letterSpacing: "0.04em", textTransform: "uppercase" as const }}>
                x{combo} COMBO
              </span>
            )}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            {started && !gameOver && (
              <span style={{ fontSize: 14, fontWeight: 700, color: timeLeft <= 5 ? "#F43F5E" : theme.text }}>
                {timeLeft}S
              </span>
            )}
            <span style={{ fontSize: 11, color: theme.textMuted, letterSpacing: "0.04em" }}>{hits}/{throws}</span>
          </div>
        </div>
            ...
            {PROJECTILES.map(p => (
              <button key={p.id} onClick={() => setAmmo(p.id)}
                style={{
                  fontSize: 16,
                  padding: "3px 6px",
                  backgroundColor: ammo === p.id ? theme.bgElevated : "transparent",
                  border: "none",
                  cursor: "pointer",
            ...
            {VENUES.map(v => (
              <button key={v.id} onClick={() => setVenue(v.id)}
                style={{
                  fontSize: 12,
                  padding: "3px 6px",
                  color: venue === v.id ? theme.text : theme.textMuted,
                  backgroundColor: venue === v.id ? theme.bgElevated : "transparent",
                  border: "none",
                  cursor: "pointer",
        ...
        <div ref={stageRef} onClick={handleClick}
          style={{
            ...
            overflow: "hidden",
            height: STAGE_H,
            backgroundColor: venueBg[venue],
            cursor: started && !gameOver ? "crosshair" : "default",
            userSelect: "none",
          }}>
          ...
          {isNeon && <>
            ...
            {[10, 30, 50, 70, 90].map((pct, i) => (
              <div key={i} style={{
                position: "absolute", top: 0, left: `${pct}%`, width: 36, height: 3, transform: "translateX(-50%)",
            ...
            {/* Neon glow */}
            <div style={{ position: "absolute", inset: 0, pointerEvents: "none", boxShadow: `inset 0 0 40px ${n1}12, inset 0 0 80px ${n2}08` }} />
          </>}
          ...
          {venue === "basement" && <>
            ...
            {/* Pipes on ceiling */}
            <div style={{ position: "absolute", top: 6, left: "20%", right: "30%", height: 3, backgroundColor: isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)" }} />
            <div style={{ position: "absolute", top: 14, left: "40%", right: "15%", height: 2, backgroundColor: isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.03)" }} />
          </>}
          ...
          {[true, false].map((isLeft, i) => (
            <div key={i} style={{
              ...
              <div style={{
                ...
                {/* Woofer */}
                <div style={{
                  position: "absolute", top: 6, left: 5, width: 26, height: 26,
                  border: `2px solid ${isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.06)"}`,
                  backgroundColor: isDark ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.01)",
                }}>
                  <div style={{ position: "absolute", top: "50%", left: "50%", width: 10, height: 10, transform: "translate(-50%, -50%)",
                    backgroundColor: isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.04)" }} />
                </div>
                {/* Tweeter */}
                <div style={{
              ...
              <div style={{
                width: 28, height: 22, marginTop: 3, marginLeft: 4,
                backgroundColor: isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.03)",
              }}>
                <div style={{ position: "absolute", top: 4, left: 5, width: 18, height: 14,
          ...
          {/* ── DJ CHARACTER — Ableton sharp style ── */}
          <div style={{
            ...
            <div style={{
              ...
              {/* Headphone pads */}
              <div style={{ position: "absolute", top: 12, left: -6, width: 7, height: 10, backgroundColor: isDark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.1)" }} />
              <div style={{ position: "absolute", top: 12, right: -6, width: 7, height: 10, backgroundColor: isDark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.1)" }} />
              ...
              {djHit ? (
                ...
                <>
                  <div style={{ position: "absolute", top: 16, left: 10, width: 4, height: 4, backgroundColor: theme.text }} />
                  <div style={{ position: "absolute", top: 16, right: 10, width: 4, height: 4, backgroundColor: theme.text }} />
                </>
              )}

              {/* Mouth — line or open when hit */}
              {djHit ? (
                <div style={{ position: "absolute", bottom: 4, left: "50%", transform: "translateX(-50%)", width: 10, height: 6, backgroundColor: "#fff" }} />
              ) : (
                <div style={{ position: "absolute", bottom: 6, left: "50%", transform: "translateX(-50%)", width: 8, height: 2, backgroundColor: isDark ? "rgba(255,255,255,0.3)" : "rgba(0,0,0,0.2)" }} />
              )}
            </div>

            {/* Left arm — sharp bar, scratching */}
            <div style={{
            ...
            {/* DJ Table — sharp rectangle with turntables */}
            <div style={{
              position: "absolute", bottom: -14, left: -16, right: -16, height: 20,
              backgroundColor: isDark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.05)",
            }}>
              {/* Turntables — square outlines */}
              <div style={{
                position: "absolute", top: 3, left: 10, width: 14, height: 14,
                border: `1.5px solid ${isDark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.08)"}`,
              }}>
                <div style={{ position: "absolute", top: "50%", left: "50%", width: 3, height: 3, transform: `translate(-50%, -50%) rotate(${bob * 50}deg)`,
                  backgroundColor: isDark ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.12)" }} />
              </div>
              <div style={{
                position: "absolute", top: 3, right: 10, width: 14, height: 14,
                border: `1.5px solid ${isDark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.08)"}`,
              }}>
                <div style={{ position: "absolute", top: "50%", left: "50%", width: 3, height: 3, transform: `translate(-50%, -50%) rotate(${bob * 50 + 90}deg)`,
                  backgroundColor: isDark ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.12)" }} />
              </div>
              {/* Mixer dots */}
              <div style={{ position: "absolute", top: 7, left: "50%", transform: "translateX(-50%)", display: "flex", gap: 3 }}>
                {[0, 1, 2].map(k => <div key={k} style={{ width: 3, height: 3, backgroundColor: isDark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.08)" }} />)}
              </div>
            </div>
          </div>
          ...
          {splats.map(s => (
            <div key={s.id} style={{ position: "absolute", left: s.x - s.size / 2, top: s.y - s.size / 2, width: s.size, height: s.size, opacity: s.opacity, pointerEvents: "none" }}>
              <div style={{ width: "100%", height: "100%", backgroundColor: s.color }} />
              {[30, 110, 190, 280].map(a => (
                <div key={a} style={{ position: "absolute", top: "50%", left: "50%", width: 4, height: 4, backgroundColor: s.color,
          ...
          {popups.map(p => (
            <div key={p.id} style={{ position: "absolute", left: p.x, top: p.y, transform: "translateX(-50%)",
              fontSize: 15, fontWeight: 800, color: theme.accent, opacity: p.opacity, pointerEvents: "none",
          ...
          {!started && (
            <div style={{
              ...
              <p style={{ fontSize: 28, fontWeight: 800, color: theme.text, marginBottom: 4, letterSpacing: "0.04em", textTransform: "uppercase" as const }}>
                TOMATO TOSS
              </p>
              <p style={{ fontSize: 13, color: theme.textSec, marginBottom: 20 }}>Throw stuff at the crappy DJ!</p>
          ...
          {gameOver && (
            <div style={{
              ...
              <p style={{ fontSize: 28, fontWeight: 800, color: theme.text, letterSpacing: "0.04em", textTransform: "uppercase" as const }}>
                {score >= 300 ? "SNIPER!" : score >= 150 ? "PAS MAL!" : score >= 70 ? "MOUAIS" : "IL DANSE MIEUX QUE TOI"}
              </p>
              <p style={{ fontSize: 20, fontWeight: 700, color: theme.text, marginTop: 8 }}>{score} PTS</p>
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 8 }}>
                <span style={{ fontSize: 13, color: theme.textSec }}>{hits}/{throws}</span>
                <span style={{ fontSize: 13, color: theme.textSec }}>{acc}%</span>
                {bestCombo >= 2 && <span style={{ fontSize: 13, color: theme.accent, fontWeight: 700 }}>BEST: x{bestCombo}</span>}
              </div>
              <button onClick={(e) => { e.stopPropagation(); startGame(); }}
                style={{
                  marginTop: 16,
