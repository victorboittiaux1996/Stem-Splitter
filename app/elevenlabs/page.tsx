...
export default function Dashboard() {
  ...
  const [urlVariant, setUrlVariant] = useState<"A" | "B" | "C" | "D">("A");
  ...
  const URL_PATTERN = /^(https?:\/\/)?(www\.)?(youtube\.com\/watch\?v=|youtu\.be\/|music\.youtube\.com\/watch\?v=|open\.spotify\.com\/track\/)/;
  const isValidUrl = URL_PATTERN.test(urlInput.trim());
                    ...
                    <div style={{ maxWidth: 900, margin: "0 auto" }}>
                      ...
                      <div className="flex items-center gap-[6px] mb-[16px]">
                        {(["A", "B", "C", "D"] as const).map(v => (
                          <button key={v} onClick={() => { setUrlVariant(v); setInputMode("file"); setUrlInput(""); }}
                            className="flex h-[28px] w-[28px] items-center justify-center rounded-[8px] text-[12px] font-semibold transition-all"
                            style={{
                              backgroundColor: urlVariant === v ? C.accent : C.bgHover,
                              color: urlVariant === v ? "#fff" : C.textMuted,
                            }}>
                            {v}
                          </button>
                        ))}
                        <span style={{ fontSize: 12, color: C.textLight, marginLeft: 8 }}>
                          {urlVariant === "A" ? "Toggle in toolbar" : urlVariant === "B" ? "URL field below" : urlVariant === "C" ? "Combined drop zone" : "Tabs above"}
                        </span>
                      </div>
                      ...
                      <div className="rounded-[16px] p-[6px]" style={{ backgroundColor: C.bgSubtle }}>
                        ...
                        {/* Variation C — combined zone (taller) */}
                        {urlVariant === "C" ? (
                          <div
                            onClick={() => !file && document.getElementById("inline-file-input")?.click()}
                            className="flex flex-col items-center justify-center transition-all duration-150 rounded-[12px]"
                            style={{ minHeight: 220, backgroundColor: C.bgCard, cursor: file ? "default" : "pointer" }}>
                            {file ? (
                              ...
                              <>
                                ...
                                <span style={{ fontSize: 13, color: C.textLight, margin: "14px 0" }}>— or —</span>
                                <div className="flex items-center gap-[8px] w-full max-w-[400px] px-[24px]" onClick={(e) => e.stopPropagation()}>
                                  <Link2 className="h-[16px] w-[16px] shrink-0" style={{ color: C.textMuted }} strokeWidth={1.6} />
                                  <input type="url" value={urlInput} onChange={(e) => setUrlInput(e.target.value)}
                                    placeholder="Paste a YouTube or Spotify link..."
                                    className="flex-1 bg-transparent outline-none"
                                    style={{ fontSize: 14, color: C.text, padding: "8px 12px", borderRadius: 8, backgroundColor: C.bgSubtle, border: `1px solid ${C.border}` }} />
                                </div>
                              </>
                            )}
                          </div>
                        ) : (
                          /* Variations A, B, D — standard drop zone or URL mode */
                          <div
                            onClick={() => {
                              const showUrlMode = (urlVariant === "A" || urlVariant === "D") && inputMode === "url";
                              if (!showUrlMode && !file) document.getElementById("inline-file-input")?.click();
                            }}
                            className="flex items-center justify-center transition-all duration-150 rounded-[12px]"
                            style={{
                              minHeight: 148,
                              backgroundColor: C.bgCard,
                              cursor: ((urlVariant === "A" || urlVariant === "D") && inputMode === "url") || file ? "default" : "pointer",
                            }}>
                            {(urlVariant === "A" && inputMode === "url") || (urlVariant === "D" && inputMode === "url") ? (
                              /* URL input mode for variants A and D */
                              <div className="flex flex-col items-center gap-[12px] py-[20px]">
                                <Link2 className="h-[20px] w-[20px]" style={{ color: C.textMuted }} strokeWidth={1.4} />
                                <input type="url" value={urlInput} onChange={(e) => setUrlInput(e.target.value)}
                                  placeholder="Paste a YouTube or Spotify link..."
                                  onClick={(e) => e.stopPropagation()}
                                  className="w-[400px] outline-none"
                                  style={{ fontSize: 14, color: C.text, padding: "8px 12px", borderRadius: 8, backgroundColor: C.bgSubtle, border: `1px solid ${C.border}`, textAlign: "center" }} />
                                {isValidUrl && (
                                  <span style={{ fontSize: 12, color: C.successText }}>Valid link detected</span>
                                )}
                              </div>
                            ) : file ? (
                              <div className="flex flex-col items-center gap-[8px]">
                      ...
                      {/* ═══ VARIATION B — URL input between drop zone and toolbar ═══ */}
                      {urlVariant === "B" && (
                        <div className="flex items-center gap-[10px] mt-[10px] px-[4px]">
                          <Link2 className="h-[15px] w-[15px] shrink-0" style={{ color: C.textMuted }} strokeWidth={1.6} />
                          <input type="url" value={urlInput} onChange={(e) => setUrlInput(e.target.value)}
                            placeholder="Paste a YouTube or Spotify link..."
                            className="flex-1 bg-transparent outline-none"
                            style={{ fontSize: 14, color: C.text, borderBottom: `1px solid ${C.border}`, paddingBottom: 6 }} />
                          {isValidUrl && (
                            <Check className="h-[14px] w-[14px]" style={{ color: C.successText }} strokeWidth={2} />
                          )}
                        </div>
                      )}
                      ...
                      <div className="flex items-center justify-between mt-[12px]">
                        <div className="flex items-center gap-[4px]">
                          ...
                          {/* ═══ VARIATION A — Toggle FILE/URL in toolbar ═══ */}
                          {urlVariant === "A" && (
                            <>
                              <div className="w-[1px] h-[16px] mx-[8px]" style={{ backgroundColor: C.textSec, opacity: 0.3 }} />
                              <div className="flex items-center rounded-[8px] overflow-hidden" style={{ border: `1px solid ${C.border}` }}>
                                {([["file", "FILE"], ["url", "URL"]] as const).map(([mode, label]) => (
                                  <button key={mode} onClick={() => setInputMode(mode)}
                                    className="px-[10px] py-[5px] text-[12px] font-semibold transition-all"
                                    style={{
                                      backgroundColor: inputMode === mode ? C.accent : "transparent",
                                      color: inputMode === mode ? "#fff" : C.textMuted,
                                    }}>
                                    {label}
                                  </button>
                                ))}
                        ...
                        <div className="flex items-center gap-[12px]">
                          ...
                          <button onClick={handleSplit} disabled={!canSplit}
                            ...
                            style={{ backgroundColor: canSplit ? C.accent : C.textLight }}>
