...
// ─── Component ──────────────────────────────────────────────
export default function AbletonDashboardV2() {
  ...
  return (
    <div ref={rootRef} className="ableton-root flex h-screen overflow-hidden" style={{ backgroundColor: C.bg, color: C.text, fontFamily: F }}>
      ...
      <aside className="flex h-full w-[220px] shrink-0 flex-col" style={{ backgroundColor: C.sidebarBg }}>
        ...
        {/* Bottom: utility + style toggle + upgrade */}
        <div style={{ backgroundColor: C.bgSubtle }} className="px-[8px] py-[6px] space-y-[1px]">
          ...
          {/* Divider */}
          <div className="mx-[10px] my-[4px]" style={{ height: 1, backgroundColor: C.textMuted, opacity: 0.15 }} />
          {/* Theme toggle */}
          ...
          <div className="relative" ref={activityRef}>
            <button onClick={() => { setActivityOpen(!activityOpen); if (!activityOpen) markAllRead(); }}
              className="flex w-full items-center gap-[10px] px-[10px] py-[9px] transition-colors relative"
              style={{ fontSize: 14, fontWeight: 500, letterSpacing: "0.04em", color: C.textSec }}>
              <RiNotificationFill size={16}/>
              ACTIVITY
              {queueItems.filter(i => i.status === "pending" || i.status === "uploading" || i.status === "processing").length > 0 && (
                <div className="flex items-center justify-center" style={{ minWidth: 14, height: 14, backgroundColor: C.accent, padding: "0 3px", marginLeft: "auto" }}>
